...
function [value,isterminal,direction]=events_adherence(tn,Qn,data)
...
value = [X23; abs(Y23)-f*abs(X23)]; % x atteint la valeur R2 - R3 TJRS FONCTION EGALE A 0 // SI plusieurs conditions d'arrets alors value est un vecteur colonne

isterminal = [1; 1]; % arret de l'intégration si l'évènement est détecté // Vaut soit 0 soit 1 // IDEM si plusieurs conditions d'arret, vecteur colonne

direction = [1; 1]; % l'évènement n'est détecté que si la fonction évènement est croissante // IDEM si plusieurs conditions d'arret, vecteur colonne
end
